...
public:
    ...
    RandNum_generator(int start, int end)
            : u(start, end), e(std::hash<std::thread::id>()(std::this_thread::get_id()) + std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count()), mStart(start), mEnd(end)
    {
    }
...
void randomRead(Engine *engine, const threadsafe_vector<std::string> &keys, unsigned numRead)
{
    ...
    for (unsigned i = 0; i < numRead; ++i)
    {
        ...
        if (key != key_from_value(val))
        {
            ...
            {
                std::cout << "Real:" << int(key_from_value(val)[xx]) << std::endl;
                std::cout << "Want:" << int(key[xx]) << std::endl;
            }
...
int main()
{
    ...
    // Write
    unsigned numWrite = 10000, numKills = 1;
    double duration = 0;
    for (int nk = 0; nk < numKills; ++nk)
    {
        std::cout << "BeginOpenEgine!" << std::endl;
        RetCode ret = Engine::Open(kEnginePath, &engine);
        std::cout << "OpenEgineSucc!" << std::endl;
        assert(ret == kSucc);
        ...
        {
            writers.emplace_back(std::thread(write, engine,
                    std::ref(keys), numWrite / numKills));
        }
        for (auto &th : writers)
        {
            th.join();
...
//    std::string result;
//    for (int i = 0; i < len; ++i) {
//        const char c = static_cast<const char>(rand() % 127);
//        result.append(&c);
//    }
//    return result;
//}
...
//int main(){
//    std::string name("/tmp");
//    auto *index = new Index(name.data());
//    for (int i = 0; i < 10000; ++i) {
//        index->add(random(8).data(),i);
//    }
//    return 0;
//}
...
//    for (int i = 0; i < len; ++i) {
//        const char c =
//                static_cast<const char>(rand() % 127);
//        result.append(&c);
//    }
//    return result;
//}
...
//    polar_race::Engine::Open("/tmp/test",&engine);
//
//    for (int i = 0; i < 100000; ++i) {
//        polar_race::PolarString key(random(8));
//        polar_race::PolarString value(random(4096));
//        engine->Write(key,value);
//        std::string v;
//        engine->Read(key,&v);
//        std::cout<<strcmp(value.data(),v.data())<<std::endl;
//    }
//
//    delete engine;
//
